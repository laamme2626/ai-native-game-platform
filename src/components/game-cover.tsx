              ...
              : isQuiz
                ...
                : isClick
                  ...
                  : isMemory
                    ...
                    : isEscape
                      ? ["from-stone-950 via-slate-700 to-orange-300", "密室", "钥匙"]
                      : ["from-blue-900 via-indigo-600 to-emerald-300", "AI 生成", "✦"];

  return (
    <div className={`relative h-[180px] overflow-hidden bg-gradient-to-br ${visual[0]} sm:h-[200px]`}>
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(15,23,42,.08),rgba(15,23,42,.55))]" />
      <div className="absolute left-4 top-4 inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-1 text-xs font-black text-slate-900 shadow-sm ring-1 ring-white/60">
        <span aria-hidden="true">{visual[2]}</span>
        <span>{visual[1]}</span>
      </div>
      <div className="absolute bottom-4 left-4 right-4">
        <p className="line-clamp-2 text-2xl font-black leading-tight text-white drop-shadow-sm">
          {title}
        </p>
      </div>
    </div>
