...
本仓库包含 AI Native 互动游戏平台 MVP 的完整源码、数据库 schema、示例数据脚本、`.env.example`、系统设计文档和交付说明文档。
...
项目提供 `.env.example`，用于列出必要环境变量。真实 `.env` 不应提交。
...
该模式不需要 API key，使用本地 fallback generator，保证评审本地可运行。
